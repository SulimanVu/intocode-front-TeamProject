  ...
  return (
    <div className={styles.back}>
      ...
      >
        ...
        <input
          className={styles.input_text}
          placeholder="id группы студента"
          value={group}
          onChange={(e) => handleGroup(e)}
        ></input>
        <input
          className={`${styles.input_text} ${styles.input_image}`}
          onChange={(e) => setImage(e.target.files[0])}
          type="file"
        ></input>
        <div>
          <button onClick={handleAddImage}>Добавить картинку</button>
        </div>
        <div>
          <button onClick={handleAddStudent}>Создать студента</button>
        </div>
      </motion.div>
    </div>
  );
